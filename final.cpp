 ...
 *
 * LCD display implemented via ILI9341 TFT + FT6206 touch
...
#define PIN_VIBRATION   A2   /* vibration sensor — HIGH = vibration */
#define PIN_BRUSH_HALL  2    /* hall switch — LOW = magnet present  */

/* DFPlayer Mini UART + busy */
#define PIN_DF_TX       4    /* Arduino TX → DFPlayer RX            */
#define PIN_DF_RX       5    /* Arduino RX ← DFPlayer TX            */
#define PIN_DF_BUSY     6    /* LOW while DFPlayer is playing        */
...
#define FSR_THRESHOLD       512   /* ADC counts (0-1023) */
...
#define BUSY_POLL_INTERVAL_MS    20   /* poll interval while waiting for DF  */
#define BUSY_TIMEOUT_MS        8000   /* safety ceiling so we never hang      */
#define MAX_SCORE               99
...
void init_audio() {
    pinMode(PIN_DF_BUSY, INPUT);             /* BUSY: LOW = playing, HIGH = idle */
    dfSerial.begin(9600);
    delay(2000);                             /* let DFPlayer boot                */
...
int read_grip_strength() { return analogRead(PIN_FSR); }

int read_grip_action() {
    return (read_grip_strength() >= FSR_THRESHOLD) ? 1 : 0;
}

int read_brush_hall() { return digitalRead(PIN_BRUSH_HALL); }

int read_vibration()  { return digitalRead(PIN_VIBRATION); }

int read_brush_action() {
    /* Both hall AND vibration must trigger together */
    return ((read_brush_hall() == LOW) && (read_vibration() == HIGH)) ? 1 : 0;
}

int read_mic_level() { return analogRead(PIN_MIC); }

int read_mic_action() {
    return (read_mic_level() >= MIC_THRESHOLD) ? 1 : 0;
}
...
void drain_sensors() {
    ...
    while ((millis() - drain_start) < (unsigned long)SHORT_DEBOUNCE_MS) {
        if (read_grip_action() || read_brush_action() || read_mic_action()) {
            drain_start = millis();   /* reset window while any sensor is still active */
        }
    }
}
 ...
 * wait_for_dfplayer()
 *
 * Blocks until PIN_DF_BUSY goes HIGH (playback finished) or
 * BUSY_TIMEOUT_MS elapses, whichever comes first.
 * Call this before any play command that must not overlap
 * the previous track (e.g. "get ready" at game start,
 * "next level" difficulty cue, win/lose stings).
 */
void wait_for_dfplayer() {
    /* Brief settling delay so the DFPlayer has time to assert BUSY
       after the previous play command before we start polling.      */
    delay(50);
 ...
 * play_track()
 *
 * Silently drops the request if the DFPlayer is still busy.
 * This prevents queuing a new track mid-playback for
 * short, non-critical cues (command prompts, good-job stings).
 */
void play_track(int track) {
    if (digitalRead(PIN_DF_BUSY) == LOW) return;   /* still playing — skip */
    dfPlayer.playMp3Folder(track);
}

void play_command_audio() {
    switch (current_command) {
        case CMD_GRIP:  play_track(TRACK_SQUEEZE); break;
        case CMD_BRUSH: play_track(TRACK_BRUSH);   break;
        case CMD_SHOUT: play_track(TRACK_SCREAM);  break;
        default: break;
    }
}
...
void set_difficulty() {
    if (score > 0 && score % 5 == 0) {
        ...
        /* Wait for current audio to finish before playing the level-up cue */
        wait_for_dfplayer();
...
void start_game() {
    ...
    wait_for_dfplayer();   /* let "get ready" finish before the first command */
...
void end_game() {
    ...
    play_lose_audio();
    set_led_color(0);
    lcd_fail();            /* doFall() inside blocks until player taps RETRY  */
    clear_leds();
}
...
void game_loop() {
    ...
    if (player_action == current_command) {
        ...
        if (score >= MAX_SCORE) {
            wait_for_dfplayer();
            play_win_audio();
            lcd_win();   /* showWinScreen() blocks until player taps RESTART */
            return;
        }
...
/* =====================================================================
 * LCD INTERFACE (called by game logic)
 * ===================================================================== */
...
void loop() {
    if (game_active) {
        /* Check for RESET button tap during gameplay */
